...
public class RequestReservationGUI extends JFrame {
	...
	public RequestReservationGUI(Pasajero d, MainGUI main)
	{
		...
		this.getContentPane().add(jButtonBook, null);
	...
	private void jButton2_actionPerformed(ActionEvent e) {
		BLFacade facade = MainGUI.getBusinessLogic();
		if (tableRides.getModel().getValueAt(tableRides.getSelectedRow(), 3)==null) {
			this.setVisible(false);
		}else {
			p= facade.bookRide((Ride)tableRides.getModel().getValueAt(tableRides.getSelectedRow(), 3), p);
			facade.setPasajeroMain(p, guardarMain);
		}
		this.setVisible(false);
	}
